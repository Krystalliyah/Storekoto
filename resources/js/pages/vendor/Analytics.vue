                ...
                <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div class="rounded-[26px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                <p class="text-sm font-medium text-[#73867F]">Revenue this week</p>
                    ...
                    <div class="rounded-[26px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                <p class="text-sm font-medium text-[#73867F]">Orders completed</p>
                                <p class="mt-2 text-2xl font-semibold text-[#183D34]">
                                    {{ totalOrders }}
                                </p>
                            </div>
                    ...
                    <div class="rounded-[26px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                <p class="text-sm font-medium text-[#73867F]">Average order value</p>
                                <p class="mt-2 text-2xl font-semibold text-[#183D34]">
                    ...
                    <div class="rounded-[26px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                <p class="text-sm font-medium text-[#73867F]">Growth signal</p>
                                <p class="mt-2 text-2xl font-semibold text-[#183D34]">+14%</p>
                            </div>
                    ...
                    <div class="grid gap-6">
                        <section class="rounded-[28px] border border-[#DCE8E1] bg-white p-5 shadow-sm sm:p-6">
                            <div class="mb-5">
                                ...
                                <h2 class="text-xl font-semibold text-[#183D34]">Weekly revenue trend</h2>
                            ...
                            <div class="grid gap-4 sm:grid-cols-7 sm:items-end">
                                <div
                                    ...
                                    :key="item.day"
                                    class="rounded-2xl border border-[#E5EEEA] bg-[#FBFCFC] p-4 sm:flex sm:h-[260px] sm:flex-col sm:justify-end"
                                >
                                    <div class="flex items-end gap-3 sm:block">
                                        <div class="sm:flex sm:h-[150px] sm:items-end">
                                            <div
                                                class="w-14 rounded-2xl bg-[#245C4A] sm:w-full"
                                                :style="{
                                        ...
                                        <div class="mt-0 flex-1 sm:mt-4">
                                            <p class="text-sm font-semibold text-[#183D34]">{{ item.day }}</p>
                                            <p class="mt-1 text-sm text-[#5F756D]">{{ formatPeso(item.revenue) }}</p>
                                            <p class="mt-1 text-xs text-[#80928B]">{{ item.orders }} orders</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <section class="rounded-[28px] border border-[#DCE8E1] bg-white p-5 shadow-sm sm:p-6">
                            <div class="mb-5">
                                ...
                                <h2 class="text-xl font-semibold text-[#183D34]">Top-selling products</h2>
                                <p class="mt-1 text-sm text-[#6E817A]">
                            ...
                            <div class="hidden overflow-hidden rounded-2xl border border-[#E4ECE8] lg:block">
                                <div class="grid grid-cols-[1.6fr_0.8fr_1fr_0.7fr] bg-[#F7FAF8] px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-[#789087]">
                                    <span>Product</span>
                                    <span>Orders</span>
                                    <span>Revenue</span>
                                    <span>Growth</span>
                                ...
                                <div class="divide-y divide-[#E7EFEB]">
                                    <div
                                        v-for="product in topProducts"
                                        :key="product.name"
                                        class="grid grid-cols-[1.6fr_0.8fr_1fr_0.7fr] items-center px-5 py-4"
                                    >
                                        <p class="font-semibold text-[#183D34]">{{ product.name }}</p>
                                        <p class="text-sm text-[#5F756D]">{{ product.orders }}</p>
                                        <p class="font-semibold text-[#183D34]">{{ formatPeso(product.revenue) }}</p>
                                        <span class="inline-flex w-fit rounded-full bg-[#EAF7F0] px-3 py-1 text-sm font-semibold text-[#1D6A4F]">
                                            {{ product.growth }}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="grid gap-4 lg:hidden">
                                ...
                                >
                                    <div class="flex items-start justify-between gap-3">
                                        <div>
                                            <p class="font-semibold text-[#183D34]">{{ product.name }}</p>
                                            <p class="mt-1 text-sm text-[#70827B]">{{ product.orders }} orders</p>
                                        </div>

                                        <span class="inline-flex rounded-full bg-[#EAF7F0] px-3 py-1 text-xs font-semibold text-[#1D6A4F]">
                                            {{ product.growth }}
                                        </span>
                                    </div>

                                    <div class="mt-4 rounded-xl bg-white p-3">
                                        <p class="text-xs uppercase tracking-[0.16em] text-[#7A8E86]">Revenue</p>
                                        <p class="mt-1 font-medium text-[#183D34]">{{ formatPeso(product.revenue) }}</p>
                                    </div>
                                </div>
                            </div>
                        </section>
                    ...
                    <aside class="grid gap-6">
                        <section class="rounded-[28px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                            <div class="mb-4 flex items-center gap-2">
                                <ShoppingBag class="h-5 w-5 text-[#245C4A]" />
                                <h3 class="text-lg font-semibold text-[#183D34]">Category mix</h3>
                            </div>

                            <div class="space-y-4">
                                <div v-for="item in categoryMix" :key="item.label">
                                    <div class="mb-2 flex items-center justify-between gap-3">
                                        <p class="text-sm font-medium text-[#355B50]">{{ item.label }}</p>
                                        <p class="text-sm font-semibold text-[#183D34]">{{ item.value }}%</p>
                                    </div>
                        ...
                        <section class="rounded-[28px] border border-[#DCE8E1] bg-white p-5 shadow-sm">
                            <div class="mb-4 flex items-center gap-2">
                                <Clock3 class="h-5 w-5 text-[#245C4A]" />
                                <h3 class="text-lg font-semibold text-[#183D34]">Peak pick-up windows</h3>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                >
                                    <div>
                                        <p class="text-sm font-medium text-[#355B50]">{{ slot.label }}</p>
                                        <div class="mt-2 h-2 rounded-full bg-[#ECF2EF]">
                                            <div
                                                class="h-2 rounded-full bg-[#245C4A]"
                                    ...
                                    <p class="text-right text-sm font-semibold text-[#183D34]">
                                        {{ slot.orders }}
                                    </p>
                                </div>
                            </div>
                        ...
                        <section class="rounded-[28px] border border-[#DCE8E1] bg-[#F7FAF8] p-5 shadow-sm">
                            <div class="mb-4 flex items-center gap-2">
                                <TrendingUp class="h-5 w-5 text-[#245C4A]" />
                                <h3 class="text-lg font-semibold text-[#183D34]">Quick insights</h3>
                            </div>
                            ...
                            <div class="space-y-3">
                                <div
                                    ...
                                    :key="insight"
                                    class="rounded-2xl border border-dashed border-[#CBD9D2] bg-white p-4 text-sm leading-6 text-[#5F756D]"
                                >
                                    {{ insight }}
                                </div>
                            </div>

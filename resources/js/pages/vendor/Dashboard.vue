...
const setupProgress = computed(() => (page.props as any).setupProgress || null);
...
<template>
  ...
  <VendorLayout>
    ...
    <div v-else class="p-6 flex flex-col gap-5">
      ...
      <!-- Store Readiness Checklist (Only if not 100%) -->
      <div v-if="setupProgress && setupProgress.percentage < 100" class="mb-2">
        <div class="overflow-hidden rounded-2xl border border-[#DCE8E1] bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div class="bg-[linear-gradient(135deg,#1B4A3D_0%,#2C725E_100%)] dark:bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_100%)] px-5 py-5 sm:px-7">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div class="mb-2 inline-flex items-center gap-2 rounded-full bg-white/12 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#F7E8C6]">
                ...
                <h3 class="text-lg font-semibold text-white tracking-tight">Complete your store profile</h3>
                <p class="text-sm text-white/80 mt-0.5">Keep your storefront polished, informative, and easy to manage.</p>
              </div>

              <div class="flex items-center gap-4">
                <div class="text-right">
                  <p class="text-xs font-semibold uppercase tracking-wider text-white/60">Progress</p>
                  <p class="text-2xl font-bold text-white leading-none mt-1">{{ setupProgress.percentage }}%</p>
                  <p class="text-[10px] text-[#F7E8C6] mt-1.5 font-medium">{{ setupProgress.completed }} of {{ setupProgress.total }} complete</p>
                </div>
                <div class="w-px h-10 bg-white/10 hidden sm:block"></div>
                <a 
                  href="/vendor/store-settings" 
                  class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#F7E8C6] px-5 py-2.5 text-sm font-bold text-[#1B4A3D] transition hover:bg-white"
                >
                  Finish setup
                </a>
              </div>
